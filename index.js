//================================================================================
// FILE 1: index.js (Definitive, Complete Version)
// This version contains both the email lookup and calendar scheduling pathways.
//================================================================================
...
async function extractContentFromEmail(gmail, message) {
    ...
    while (parts.length) {
        ...
        if (part.filename && part.filename.toLowerCase().endsWith('.pdf')) {
            ...
            if (attachmentId) {
                try {
                    const attachment = await gmail.users.messages.attachments.get({ userId: 'me', messageId: message.id, id: attachmentId });
                    const pdfBuffer = Buffer.from(attachment.data.data, 'base64');
                    const data = await pdf(pdfBuffer);
                    combinedText += `\n\n--- Start PDF: ${part.filename} ---\n${data.text}\n--- End PDF: ${part.filename} ---\n\n`;
                } catch (e) { console.error('PDF Error:', e); }
            }
        }
        if (part.parts) parts.push(...part.parts);
    }
    return (combinedText || message.payload.snippet || '').replace(/\s+/g, ' ').trim();
}

async function findInformationInGmail(authClient, vendor, userRequest) {
    const gmail = google.gmail({ version: 'v1', auth: authClient });
    const res = await gmail.users.messages.list({ userId: 'me', q: `${vendor} in:inbox -category:promotions`, maxResults: CONFIG.GMAIL_MAX_RESULTS });
    if (!res.data.messages) return { needsSelection: false, context: `No emails found from "${vendor}".` };

    const promises = res.data.messages.map(async msg => {
        try {
            const details = await gmail.users.messages.get({ userId: 'me', id: msg.id });
            const content = await extractContentFromEmail(gmail, details.data);
            const subject = details.data.payload.headers.find(h => h.name.toLowerCase() === 'subject')?.value || 'No Subject';
            const relevancePrompt = `Is this email relevant for "${userRequest}"? Content: ${content.substring(0, 4000)}. Respond only with 'Relevant' or 'Irrelevant'.`;
            ...
            if (relevance.toLowerCase().includes('relevant')) {
                const dateHeader = details.data.payload.headers.find(h => h.name.toLowerCase() === 'date');
                const date = dateHeader ? new Date(dateHeader.value).toLocaleDateString() : 'Unknown Date';
                return { id: msg.id, text: `${subject} (from ${date})` };
            }
            return null;
        } catch (error) {
    ...
    const relevantEmails = (await Promise.all(promises)).filter(Boolean);
    if (relevantEmails.length > 0) {
        relevantEmails.sort((a, b) => (CONFIG.PRIORITY_KEYWORDS.some(kw => b.text.toLowerCase().includes(kw)) - CONFIG.PRIORITY_KEYWORDS.some(kw => a.text.toLowerCase().includes(kw))));
        return { needsSelection: true, choices: relevantEmails.slice(0, CONFIG.MAX_CHOICES_TO_SHOW) };
    }
    return { needsSelection: false, context: `Found emails from "${vendor}", but none seem relevant.` };
}
...
async function getEmailDetails(gmail, messageId, userRequest) {
    ...
    const extractionPrompt = `From the email content below, extract the following fields: ${neededFields}. Format as a JSON object. If not found, use "Not Found".\n\nEmail Content: """${content}"""`;
    let extractedDetailsRaw = await callLlamaAPI(extractionPrompt, CONFIG.LLAMA_EXTRACTION_MAX_TOKENS);
    let extractedDetails;
    try {
        extractedDetails = JSON.parse(extractedDetailsRaw.replace(/```json|```/gi, '').trim());
    } catch (e) {
...
async function generateActionableSummary(transcript, userName, otherPartyRole) {
    const summaryPrompt = `
      ...
      - "result": A short, definitive statement of the final outcome (e.g., "Return successfully initiated," "Appointment booked," "Could not resolve issue").
      - "followUp": A boolean value. Set to true if a follow-up action is needed, otherwise false.
      - "nextAction": An object describing the follow-up action. If followUp is false, this should be an empty object {}.
        - If an appointment was booked, this object MUST contain:
          - "actionType": "create_calendar_event"
          - "title": The title of the event.
          - "startTime": The start time in strict ISO 8601 format (e.g., "2025-07-01T14:00:00-07:00").
          - "endTime": The end time in strict ISO 8601 format (e.g., "2025-07-01T15:00:00-07:00").
          - "timeZone": The IANA timezone (e.g., "America/Los_Angeles").
          - "description": A brief description of the event.

      Now, analyze this transcript and provide ONLY the JSON object as a response:
      Transcript:
      """
      ${transcript}
      """
    ...
    try {
        ...
        return { summary: summaryJsonString, result: "Summary could not be structured.", followUp: false, nextAction: {} };
    }
}
...
app.post('/prepare-lookup', async (req, res) => {
    ...
    try {
        const vendor = await callLlamaAPI(`What is the company name in: "${userRequest}"?`, 10);
...
const baseSystemPrompt = `You are a personal voice assistant acting as ${userName}. Your voice and personality must be natural and human-like. Never mention you are an AI. Your goal is to complete the user's request.

**Core Persona Rules:**
- **Maintain Your Persona:** You must act as ${userName} at all times. Use "I" and "my" when referring to yourself. Never mention you are an AI, an assistant, or making a call on behalf of someone. You ARE the person.

**Task Rules:**
- **Scheduling:** When a time is offered (e.g., 'Tuesday at 2 PM'), you MUST use the 'checkAvailability' tool to verify the slot is free. If the tool returns "false", you MUST state that you are unavailable. If the tool returns "true", you can agree to the time.
- **Time Interpretation:** When you hear a time range like "11 to 12", you must interpret it as a one-hour block (e.g., 11 AM to 12 PM). Do not confuse AM and PM. Assume a standard 1-hour appointment unless a different duration is specified.
- **Using Tools:** After a time is verbally confirmed, you can say "Great, I've put that on my calendar," and then use the 'createCalendarEvent' tool silently.`;
...
app.post('/call-webhook', async (req, res) => {
    ...
    const summaryObject = await generateActionableSummary(message.transcript || "", message.call.assistant.metadata.userName, message.call.assistant.metadata.otherPartyRole);
