...
app.use(
    session({
        secret:
            process.env.SESSION_SECRET || "a-very-secret-key-for-your-session",
        resave: false,
...
async function callLlamaAPI(
    prompt,
    max_tokens = CONFIG.LLAMA_NEEDED_INFO_MAX_TOKENS,
) {
    ...
    if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        console.error(
            "Unexpected Llama API response format:",
        ...
        if (part.filename && part.filename.toLowerCase().endsWith(".pdf")) {
            ...
            if (attachmentId) {
                try {
                    ...
                    console.error(
                        `Failed to parse PDF ${part.filename}:`,
                        pdfError,
                    );
...
async function findInformationInGmail(authClient, vendor, userRequest) {
    ...
    const emailProcessingPromises = messages.map(async (msg) => {
        try {
            ...
            const content = await extractContentFromEmail(
                gmail,
                messageDetails.data,
            );
            const subject =
...
Based on these rules, is this email transactional and relevant? Respond with only the single word: "Relevant" or "Irrelevant".`;
            ...
            const relevance = await callLlamaAPI(
                relevancePrompt,
                CONFIG.LLAMA_RELEVANCE_MAX_TOKENS,
            );

            console.log(
                `Email Subject: "${subject}" -> Relevance: ${relevance}`,
            );
    ...
    if (relevantEmails.length > 0) {
        ...
        console.log(
            "Prioritizing relevant emails before showing final list...",
        );
        relevantEmails.sort((a, b) => {
            const aSubject = a.text.toLowerCase();
            const bSubject = b.text.toLowerCase();
            const aHasPriority = CONFIG.PRIORITY_KEYWORDS.some((kw) =>
                aSubject.includes(kw),
            );
            const bHasPriority = CONFIG.PRIORITY_KEYWORDS.some((kw) =>
                bSubject.includes(kw),
            );
        ...
        console.log(
            "--- Final Sorted List of Relevant Emails (before slicing) ---",
        );
...
async function getEmailDetails(gmail, messageId, userRequest) {
    ...
    const neededFields = await callLlamaAPI(
        neededInfoPrompt,
        CONFIG.LLAMA_NEEDED_INFO_MAX_TOKENS,
    );
    ...
    let extractedDetailsRaw = await callLlamaAPI(
        extractionPrompt,
        CONFIG.LLAMA_EXTRACTION_MAX_TOKENS,
    );

    // Sanitize and parse the JSON
    let extractedDetails;
    try {
        extractedDetailsRaw = extractedDetailsRaw
            .replace(/```json|```/gi, "")
            .trim();
    ...
    let phoneNumberFromEmail = await callLlamaAPI(
        phonePrompt,
        CONFIG.LLAMA_PHONE_MAX_TOKENS,
    );
...
async function generateDetailedSummary(transcript, userName, otherPartyRole) {
    if (!transcript)
        return "Call ended, but no transcript was available to generate a summary.";
    const summaryPrompt = `
...
app.post("/search-email", async (req, res) => {
    ...
    try {
        ...
        const vendor = await callLlamaAPI(
            vendorPrompt,
            CONFIG.LLAMA_VENDOR_MAX_TOKENS,
        );
        const emailResult = await findInformationInGmail(
            oauth2Client,
            vendor,
...
- Once the task is complete, end the call politely.`;
    ...
    try {
                    ...
                    tools: [
                        {
                            ...
                            function: {
                                name: "sendDTMF",
                                description:
                                    "Sends a DTMF tone during the call to navigate phone menus.",
                                parameters: {
                                    type: "object",
        ...
        if (!vapiResponse.ok) {
            ...
            throw new Error(
                `Vapi API Error: ${JSON.stringify(errorData.message || errorData)}`,
            );
        }
...
app.post("/stop-call", async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            message: "Call stop request sent successfully.",
        });
    } catch (error) {
...
app.post("/call-webhook", async (req, res) => {
    ...
    if (message.type === "end-of-call-report") {
        ...
        const otherPartyRole =
            message.call.assistant.metadata.otherPartyRole || "The Other Party";
        ...
        if (message.role === "assistant") {
            callStatuses[callId] =
                `${message.call.assistant.metadata.userName || "Agent"} is speaking...`;
